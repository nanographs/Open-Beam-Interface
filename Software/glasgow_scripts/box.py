...
def box(x_start, y_start, x_width, y_height):
    ...
    for x in range(x_start, x_start+x_width):
        seq.add(VectorPixelCommand(x_coord=x, y_coord = y_start, dwell_time = dwell))
        print(f"{x=}")
    for y in range(y_start, y_start+y_height):
        seq.add(VectorPixelCommand(x_coord=x_start+x_width, y_coord = y, dwell_time = dwell))
        print(f"{y=}")
    for x in range(x_start+x_width, x_start, -1):
        seq.add(VectorPixelCommand(x_coord=x, y_coord = y_start+y_height, dwell_time = dwell))
        print(f"{x=}")
    for y in range(y_start+y_height, y_start, -1):
        seq.add(VectorPixelCommand(x_coord=x_start, y_coord = y, dwell_time = dwell))
        print(f"{y=}")
