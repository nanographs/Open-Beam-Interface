...
import numpy as np
import asyncio
from ..beam_interface import Connection, RasterStreamCommand, DACCodeRange, ExternalCtrlCommand, BeamType
from ..frame_buffer import FrameBuffer

im = Image.open("amaranth-logo.png")
im = im.convert("L")
...
async def stream_pattern():
    ...
    await conn.transfer(ExternalCtrlCommand(enable=1, beam_type=BeamType.Ion))
    async for res in conn.transfer_multiple(RasterStreamCommand(cookie=123, x_range = x_range, y_range = y_range, dwells=pixels), latency=1000000000):
        pass
    full_frame = None
