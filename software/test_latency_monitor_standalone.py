#!/usr/bin/env python3
"""
Simple test script for the latency monitor functionality.
This script tests the latency monitor without importing the full OBI system.
"""
...
class LatencyVisualizationWidget(QWidget):
    ...
    def setup_realtime_tab(self):
        ...
        # Main latency plot
        self.main_plot = self.plot_widget.addPlot(row=0, col=0)
        ...
        # Network detail plot
        self.network_plot = self.plot_widget.addPlot(row=1, col=0)
        self.network_plot.setTitle("Network Latency Detail")
        ...
        self.network_receive_curve = self.network_plot.plot(
    ...
    def setup_stats_tab(self):
        ...
        # Headers
        headers = ['Stage', 'Mean (ms)', 'Median (ms)', 'P95 (ms)', 'P99 (ms)', 'Max (ms)', 'Min (ms)']
        for i, header in enumerate(headers):
            label = QLabel(header)
            label.setStyleSheet("font-weight: bold; background-color: #ecf0f1; padding: 5px;")
            self.stats_grid.addWidget(label, 0, i)
        
        # Create labels for each stage
        self.stats_labels = {}
        for i, stage in enumerate(self.latency_monitor.latency_history.keys(), 1):
            stage_label = QLabel(stage.replace('_', ' ').title())
            stage_label.setStyleSheet("font-weight: bold; color: #2c3e50;")
            self.stats_grid.addWidget(stage_label, i, 0)
            
            self.stats_labels[stage] = {}
            for j, stat in enumerate(['mean', 'median', 'p95', 'p99', 'max', 'min'], 1):
                label = QLabel("0.0")
                label.setStyleSheet("padding: 3px; border: 1px solid #bdc3c7;")
                self.stats_grid.addWidget(label, i, j)
                self.stats_labels[stage][stat] = label
    ...
    def update_plot_data(self, stage: str, latency: float):
        ...
        # Update network detail plot
        if stage == 'network_send':
            recent_data = self.latency_monitor.get_recent_latencies(stage, 50)
            if recent_data:
                time_axis = np.linspace(current_time - len(recent_data) * 0.1, current_time, len(recent_data))
                self.network_send_curve.setData(time_axis, recent_data)
        
        elif stage == 'network_receive':
            recent_data = self.latency_monitor.get_recent_latencies(stage, 50)
            if recent_data:
                time_axis = np.linspace(current_time - len(recent_data) * 0.1, current_time, len(recent_data))
                self.network_receive_curve.setData(time_axis, recent_data)
    ...
    def update_statistics(self):
        ...
        for stage in self.latency_monitor.latency_history.keys():
            ...
            if stage in self.stats_labels:
                for stat_name, value in stats.items():
                    if stat_name in self.stats_labels[stage]:
                        self.stats_labels[stage][stat_name].setText(f"{value:.2f}")
